...
def create_skill_package(lang_code, lang_name):
    ...
    Args:
        ...
        lang_name: 语言名称 ('ForkIt_zh' 或 'ForkIt_en')
    ...
    with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
        # 添加 SKILL.md（放在 fork-it/ 目录下）
        if skill_md_path.exists():
            zipf.write(skill_md_path, "fork-it/SKILL.md")
            print(f"  ✓ 添加 fork-it/SKILL.md")
        else:
            raise FileNotFoundError(f"找不到文件: {skill_md_path}")

        # 添加 _meta.json（放在 fork-it/ 目录下）
        if meta_json_path.exists():
            zipf.write(meta_json_path, "fork-it/_meta.json")
            print(f"  ✓ 添加 fork-it/_meta.json")
        else:
            raise FileNotFoundError(f"找不到文件: {meta_json_path}")

        # 添加 scripts 目录及其内容（放在 fork-it/scripts/ 目录下）
        if scripts_dir.exists() and scripts_dir.is_dir():
            for file_path in scripts_dir.iterdir():
                if file_path.is_file():
                    arc_name = f"fork-it/scripts/{file_path.name}"
